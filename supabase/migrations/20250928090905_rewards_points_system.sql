...
do $$ begin
  if exists (select 1 from information_schema.tables where table_name = 'shop_points_ledger' and table_schema = 'public') then
    -- Table exists, add missing columns
    if not exists (select 1 from information_schema.columns where table_name = 'shop_points_ledger' and column_name = 'reference_type') then
      alter table public.shop_points_ledger add column reference_type text check (reference_type in ('case_code', 'redemption_id', 'manual'));
      alter table public.shop_points_ledger add column reference_id text;
      alter table public.shop_points_ledger add column product_id uuid references public.products(id) on delete set null;
      alter table public.shop_points_ledger add column variant_id uuid references public.product_variants(id) on delete set null;
      alter table public.shop_points_ledger add column campaign_id uuid;
      alter table public.shop_points_ledger add column status text not null default 'completed' check (status in ('completed', 'reversed'));
      alter table public.shop_points_ledger add column created_by uuid default auth.uid();
    end if;
  end if;
end $$;
...
-- Note: This will be created after confirming table structure
...
create policy "points_rules_admin_all" on public.points_rules
    ...
    with check (coalesce((current_setting('request.jwt.claims', true)::jsonb ->> 'role'),'')
                in ('hq_admin', 'power_user'));
...
-- Note: Shop-specific access is checked at API layer via get_my_shop_id()
...
stable
as $$
  -- For shop users, get their shop from shop_distributors
  select sd.shop_id
  from public.shop_distributors sd
  where sd.distributor_id in (
    select mu.manufacturer_id
...
begin
  ...
  where shop_id = p_shop_id;
                          ...
                          if v_current_balance < v_catalog.points_cost then
                            raise exception 'Insufficient points: have %, need %', v_current_balance, v_catalog.points_cost;
                          end if;  -- Insert redemption (pending)
